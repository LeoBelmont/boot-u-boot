...
static int cdns_xspi_configure_phy(struct cdns_xspi_priv *priv)
{
	...
	writel(0x00000404 | (INTERNAL_LPBK_DQS << LPBK_DQS_SET_POS),
			priv->regbase + CDNS_XSPI_PHY_DATASLICE_RFILE_PHY_DQS_TIMING);
	writel(0x00000030 | (rd_dly << RD_DEL_SEL_POS),
			priv->regbase + CDNS_XSPI_PHY_DATASLICE_RFILE_PHY_GATE_LPBK_CTRL);
