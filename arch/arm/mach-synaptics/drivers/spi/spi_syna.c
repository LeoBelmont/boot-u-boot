...
static int syna_spi_claim_bus(struct udevice *dev)
{
	...
	struct syna_spi_platdata *plat = dev_get_plat(bus);
	struct syna_spi_regs *reg = plat->reg;
...
static int syna_spi_of_to_plat(struct udevice *bus)
{
	int ret;
	struct syna_spi_platdata *plat = dev_get_plat(bus);
...
U_BOOT_DRIVER(syna_spi) = {
	...
	.ops = &syna_spi_ops,
	.of_to_plat = syna_spi_of_to_plat,
	.plat_auto = sizeof(struct syna_spi_platdata),
	.priv_auto = sizeof(struct syna_spi_priv),
