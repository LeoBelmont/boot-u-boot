...
int diag_config_pll_ssc(unsigned int pllbase, int ssc_freq, int ssc_amp, int ssc_mode, int enable)
{
	...
	if (enable) {
		...
		debug("written SSC depth is: %.1f,  input SSC depth is: %.1f\n", wssc_amp,
		      (unsigned long long)ssc_amp / 10);
#else
		PRINT("written SSC depth is: %d,  input SSC depth is: %d\n", (int)(wssc_amp * 10),
		      ssc_amp);
#endif
		fpfd = diag_config_pll_getfpfd(pllbase);
		// per SPEC page 8, example in Table5B, SSC_freq = Fpfd/SSMF[3:0]
...
// integer freq change
int diag_change_pll(E_PLL_SRC pll, int freq, int freq1, int ssc, int ssc_freq, int ssc_amp,
		    int ssc_mode, int calc, APLLCFG_t apll_cfg)
{
	int ret = 0;
...
int diag_change_avpll(unsigned int apll_index, unsigned int freq, unsigned int ssc,
		      unsigned int ssc_freq, unsigned int ssc_amp, unsigned int ssc_mode,
		      unsigned int calc, APLLCFG_t pll_cfg)
{
	...
	ret = diag_change_pll(VPLL0 + apll_index, freq, freq, ssc, ssc_freq, ssc_amp, ssc_mode,
			      calc, pll_cfg);
