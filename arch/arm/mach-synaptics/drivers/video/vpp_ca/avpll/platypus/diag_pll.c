...
int diag_config_pll_getpfdrange(unsigned int pllbase)
{
	 ...
	 * For platypus current config,fpfd comes as 5 for LCD,6 for HDMI
	 */
	// 8 possible ranges
	if (fpfd >= 5 && fpfd < 7) {
		r = 0;
...
int diag_config_pll_ssc(unsigned int pllbase, int ssc_freq, int ssc_amp, int ssc_mode, int enable)
{
	...
	unsigned long long wssc_amp, fpfd, ssc_freqd;
		      ...
		      unsigned int calc, APLLCFG_t pll_cfg)
{
	...
	ret = diag_change_pll(apll_index + 5, freq, freq, ssc, ssc_freq, ssc_amp, ssc_mode, calc,
			      pll_cfg);
