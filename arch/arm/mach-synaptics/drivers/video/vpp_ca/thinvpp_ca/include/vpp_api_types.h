...
typedef struct tagVPP_HDMI_PKT_VNDRSPEC_INFOFRM {
	UINT32    ieeeRegID;
	UINT8     PacketLength;
	UINT8     HdmiVideoFmt; //defines the structure of the extended video formats -
				//VPP_HDMI_EXTENDED_VIDEO_FORMAT
	...
	BOOL      Hdmi_3D_Meta_Present; //additional bytes of 3D Metadata are
					//present in the Infoframe
	UINT8     Hdmi_3D_Ext_Data;
	UINT8     Hdmi_3D_Metadata_Type;//Type of info of metadata whcich is used
					//in the correct rendering of the stereoscopic video
...
typedef enum tag_VPP_HDMI_SINKCAP_BITMASK {
...
typedef struct VPP_HDMI_SINK_CAPS_T {
	...
	unsigned int      sprted3DDispRes; // bit[x]: 1 - support, 0 - not support.
					   // x means for 3D resolution 3D_FIRST_3D+x
	unsigned int      sprted3DStructures[/*MAX_NUM_RES_3D-FIRST_RES_3D+1*/32];
			// sprted3DStructures[x]: x means for 3D resolution 3D_FIRST_3D+x
			// bit[15~0]: bit[x] set to 1 means 3D structure x is supported,
