...
/* Enable EN_VPP_DYNAMIC_CLKGATE to dynamically clock gate various IPs in VPP
 * based on if they are using or soft bypass mode
 */
#define EN_VPP_DYNAMIC_CLKGATE
#endif
#ifdef EN_VPP_DYNAMIC_CLKGATE
/* We can do SRAM gating only if respective block is clk gated
 * SRAM gating is available in BG4_CDp/CDp_A0
 */
...
#define VPP_FRC_SCL_LAY_MAX      4 /* Max among all the scalers */
#else
#define VPP_FRC_SCL_MAIN_LAY_MAX 2 /* Main and VIP SCL */
#define VPP_FRC_SCL_GFX_LAY_MAX  2 /* PIP and OSD */
#define VPP_FRC_SCL_LAY_MAX      2 /* Max among all the scalers */
#endif
#define VPP_FRC_SCL_NUM_OF_PHASES 17
#define VPP_FRC_SCL_NUM_OF_COEFF  8
#define VPP_FRC_SCL_COEFF_TAB_SIZE \
	(VPP_FRC_SCL_NUM_OF_PHASES * VPP_FRC_SCL_NUM_OF_COEFF * sizeof(UINT16))
...
#define VPP_FRC_COEFFTAB_BCMBUF_NUM_OF_PAIRS \
	(VPP_FRC_COEFFTAB_BCMBUF_NUM_OF_PAIRS_PERPHASE * VPP_FRC_SCL_NUM_OF_PHASES)
#define VPP_FRC_COEFFTAB_BCMBUF_SIZE_PERPHASE \
	(VPP_FRC_COEFFTAB_BCMBUF_NUM_OF_PAIRS_PERPHASE * sizeof(UINT32))
#define VPP_FRC_COEFFTAB_BCMBUF_SIZE \
	(VPP_FRC_COEFFTAB_BCMBUF_SIZE_PERPHASE * VPP_FRC_SCL_NUM_OF_PHASES)
 ...
 * based on VDEC is sending (stride*4) or (stride*1)
 * in m_buf_stride & m_buf_stride_UV
...
/* Enable to croma aligned linedropping when full FETG cropping is available.
 * Eg. 2 contiguous Y lines and corresponding 1 UV alines for 2x line droppping
 * and smilarly for other linedrops
 */
...
#define PRINT_WIN(planeID, win) \
	VPPAPI_DBG_INFO(VPP_DEBUG_CROP_PREFIX \
			" %s : plane [%d], x[%d], y[%d], width[%d], height[%d] \n", \
			__FUNCTION__, planeID, (win)->x, (win)->y, (win)->width, (win)->height)
#define PRINT_WIN1(win) \
	VPPAPI_DBG_INFO(VPP_DEBUG_CROP_PREFIX \
			" %s : Win [%s], x[%d], y[%d], width[%d], height[%d] \n", \
...
typedef enum {
	...
	MV_VPP_ENOTIMPL       = 0x0001,   /**< Function/feature not implemented. */
...
#if (BERLIN_CHIP_VERSION >= BERLIN_VS680)
#define IS_GFX_SRC_FORMAT(src_fmt) \
	(((src_fmt) == SRCFMT_XRGB32) || ((src_fmt) == SRCFMT_ARGB32_PM) || \
	 ((src_fmt) == SRCFMT_ARGB32) || ((src_fmt) == SRCFMT_IPT444) || \
	 ((src_fmt) == SRCFMT_ARGB1555) || ((src_fmt) == SRCFMT_ARGB4444) || \
	 ((src_fmt) == SRCFMT_ARGB8332) || ((src_fmt) == SRCFMT_RGB565) || \
	 ((src_fmt) == SRCFMT_ARBG2101010) || ((src_fmt) == SRCFMT_RGB888))

#define IS_GFX_AMP_FORMAT(src_fmt) \
	(((src_fmt) == AMP_DISP_SRCFMT_XRGB32) || ((src_fmt) == AMP_DISP_SRCFMT_ARGB32_PM) || \
	 ((src_fmt) == AMP_DISP_SRCFMT_ARGB32) || ((src_fmt) == AMP_DISP_SRCFMT_IPT444) || \
	 ((src_fmt) == AMP_DISP_SRCFMT_ARGB1555) || ((src_fmt) == AMP_DISP_SRCFMT_ARGB4444) || \
...
/* Bit mask for dolby vision configurature on Base layer present.
 * it shall be on (1) when dolby vision present
 */
...
/* Bit mask for dolby vision configurature on Meta data present.
 * it shall be on (1) when dolby vision present
 */
#define VPP_DOLBY_VISION_RPU (1 << 2)
/* Bit mask for dolby vision configuration for low latency.
 * It shall be (1) for dolbyvision low latency stream
 */
...
typedef enum VPP_UNIQUE_FORMAT_ID {
	...
	VPP_UNIQUE_FORMAT_ID_16BIT_ARGB4444     =   15,
	/*this should match with VIDEO_DATA_Y_ALIGN_BYTES in vpp_cpcb_ovl.c */
...
/*CGMS copy control information transfer mode*/
typedef enum {
...
/*CGMS copy control information mode*/
typedef enum {
...
#define IS_VPP_CLKGATE_DISABLE(clkGateOpt, OPTION) \
	((clkGateOpt) & VPP_CLKGATE_ENABLE_OPT_DISABLE_##OPTION)
#define IS_VPP_CLKGATE_DBG_ENABLE(clkGateOpt, OPTION) \
	((clkGateOpt) & VPP_CLKGATE_ENABLE_OPT_DBG_##OPTION)
...
typedef enum __ENUM_VPP_CLKGATE_OPT__ {
	...
	VPP_CLKGATE_DLY_COUNT_MASK = 0x00FF0000,// User defined delay in terms of interrupts,
						// otherwise delay count will be used
...
typedef struct VPP_WIN_ATTR_T {
	...
	ENUM_GLOBAL_ALPHA_FLAG globalAlphaFlag; //1:enable plane/global alpha
						// otherwise use per pixel alpha
} VPP_WIN_ATTR;
...
#define VPP_DV_FRAME_DUMP_MAX_USED_BITS_WORDS \
	((VPP_DV_FRAME_DUMP_MAX_FRAMES + \
	  VPP_DV_FRAME_DUMP_MAX_USED_BITS_PER_WORD - 1) / VPP_DV_FRAME_DUMP_MAX_USED_BITS_PER_WORD)
...
typedef struct VPP_DV_FRAME_DUMP_T {
	...
	unsigned int skipFrames;        //Number of frames to be skipped
					// before initiating framedump
	unsigned char overwriteFlag : 1;//Overwrite the frame buffer upon consuming all buffers
	unsigned char forcecapture  : 1;//Force capture flag for enable builtin frame check
					// for DV capture cases
	unsigned char reserve : 6;

	//Internal or output parameters
	unsigned int currentNdx;//current read/write pointer of buffer, one behind the nextNdx
	unsigned int nextNdx;	//Next read/write pointer of buffer,
				// update with every new frame received from inputq
	unsigned int usedFrames[VPP_DV_FRAME_DUMP_MAX_USED_BITS_WORDS]; //frame dump status bit
								//1st world LSB:0, 2nd word LSB:32
 ...
 * RETURN: NONE
 **************************************/
int MV_VPPOBJ_Config(int handle, const int *pvinport_cfg, const int *pdv_cfg,
		     const int *pzorder_cfg, const int *pvoutport_cfg, const int *pfeature_cfg);

// to be removed when VPP suspend/resume is ready
int MV_VPP_GetPlaneInfo(int handle, int planeID, uintptr_t *curr_frame,
			uintptr_t *curr_still_picture, int *mute);
 ...
 * RETURN: MV_VPP_OK - SUCCEED
 *******************************************************************/
int MV_VPPOBJ_SetScalerNonLinearControl(int handle, int planeID, int enable, int center_frac,
					int center_ratio);
 ...
 * INPUT:  MUX VALUE for Ch3Ch2Ch1Ch0 [Input value of Hex where each Nibble corresponds
 *	   to each channel]
 * RETURN: MV_VPP_OK - SUCCEED
 *         MV_EBADPARAM - invalid parameters
 *         MV_EUNCONFIG - VPP not configured
...
/********************************************************************************
 * FUNCTION: Send HDMI specified auxiliary packets
 * INPUT: pktType - Packet Type
 *      : pAuxPkt - Pointer to auxiliary packet data
...
/********************************************************************************
 * FUNCTION: Stop HDMI auxiliary packets
 * INPUT: pktType - Packet Type
 * RETURN: MV_VPP_OK - SUCCEED
 *         MV_EBADPARAM - invalid parameters
 *         MV_EUNCONFIG - VPP not configured
 ...
 * VPP_CLKGATE_OPT_RECORD_RAW - Enable storing raw bit from the register into corresponding bit of
 *				IP block in signatrue
 *		Default behaiour: Enabling bit correspoing to IP block in signatrue
 *		only if clock gated/disabled
 * OUTPUT: pSignatureCount, The number of IP blocks which are clock gated/disabled
 *         pSignature, 128 bit signature (1 bit per IP block),
 *	   ensure it points to array of 4 integers
 *
 * RETURN: MV_VPP_OK - SUCCEED
 *         MV_EBADPARAM - invalid parameters
 *         MV_EUNCONFIG - VPP not configured
 ...
 * VPP_CLKGATE_OPT_RECORD_RAW - Enable storing raw bit from the register into corresponding bit of
 *	IP block in signatrue
 *      Default behaiour: Enabling bit correspoing to IP block
 *	in signatrue only if clock gated/disabled
 *      pSignatureValue, 128 bit signature value(1 bit per IP block),
 *	ensure it points to array of 4 integers
 *      pSignaturemASK, 128 bit signature mask(1 bit per IP block),
 *	ensure it points to array of 4 integers
 *
 * RETURN: MV_VPP_OK - SUCCEED
 *         MV_EBADPARAM - invalid parameters
...
int MV_VPPOBJ_DV_InitFrameDump(int handle, INT Path, INT Enable,
			       VPP_DV_FRAME_DUMP *pFrameDumpConfig);
