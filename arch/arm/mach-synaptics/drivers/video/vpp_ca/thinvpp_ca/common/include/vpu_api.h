...
//---------------------------------------------------------------------------
// \brief      Basic Constant Parameters of Video Encoder (read-only)
// //Obsolete, for backward compatibity only
//---------------------------------------------------------------------------
TYPEDEF_STRUCT(venc_const_t)
{
	...
	unsigned int    venc_hw_context_size;	// Size (in bytes) of the video decoder context
						// for hardware.
	unsigned int    rsvd0;                // Reserved.
	unsigned int    rsvd1;                // Reserved.
...
{
	void            *user_data;	// User-defined decoder context data,
					// not used by the video decoder.
	...
	unsigned int    hw_context_addr;// Base address (4KB-aligned) of the video decoder
					// context buffer for hardware.
	void            *hw_context_vaddr; // Virtual address (4KB-aligned) of the video decoder
					// context buffer for hardware.
	void            *io_rd32;          // Function pointer for 32-bit I/O read.
	void            *io_wr32;          // Function pointer for 32-bit I/O write.

	void            *external_clock_disable;// Function pointer for external_clock_disable
						// 1: disable; 0: enable.
	void            *reg32_read;	// Function pointer to access 32-bit register space.
					//Reserved for future improvements
	void            *reg32_write;	// Function pointer to access 32-bit register space.
					//Reserved for future improvements
	void            *reg32_fill;	// Function pointer to access 32-bit register space.
					//Reserved for future improvements
	void            *reg32_copy;	// Function pointer to access 32-bit register space.
					//Reserved for future improvements
	void            *mem32_read;	// Function pointer to access 32-bit physical memory.
					// Reserved for future improvements
	void            *mem32_write;	// Function pointer to access 32-bit physical memory.
					// for cabac opt
	void            *mem32_fill;	// Function pointer to access 32-bit physical memory.
					// Reserved for future improvements
	void            *mem32_copy;	// Function pointer to access 32-bit physical memory.
					// Reserved for future improvements

	void            *sl2_buf_drain;     // Function pointer to drain SL2 store buffer;
	void            *before_asic_start; // Function pointer for do sth. before asic start
	...
	dma_drv_cfg_t   dma_drv_cfg;	// Function pointer sets for
					// external fake DMA driver implementation;
...
TYPEDEF_STRUCT(vdec_strm_config_t)
{
	...
	void           *base_strm;      // Pointer to the stream context of the base stream.
					// Only used in H.264 MVC.
	unsigned int    format;         // Stream format ID.

	unsigned int    hw_context_addr;// Base address (4KB-aligned) of the
					// stream context buffer for hardware.
	void           *hw_context_vaddr;// Virtual address (4KB-aligned) of the
					// stream context buffer for hardware.
	...
	unsigned int    mon_width;      // Preferred display buffer/monitor width in pixels.
					// 0 = auto.
	unsigned int    mon_height;     // Preferred display buffer/monitor height in pixels.
					// 0 = auto.
	...
	unsigned int    pp_vscale;      // Vertical down-sampling ratio 0 or 1: no scaling,
					// N: 1/N: 2: 1/2, 3: 1/3, 4: 1/4, up to 1/16
	unsigned int    pp_hscale_numer;// Horizontal down-scaling ratio of numerator
					// (support from BG4CDP-A0)
	unsigned int    pp_hscale_denom;// Horizontal down-scaling ratio of denominator
					// (support from BG4CDP-A0)
	...
	unsigned int    disable_vld_dcg;    // Flag to  disable VLD module's dynamic clock gating:
						// temporary flag for V2.2
	...
	unsigned int    roi_x;  // ROI (region of interest) start location in horizontal direction
				// (after down-scaling, unit in pixel) (JPEG only)
	unsigned int    roi_y;  // ROI start location in vertical direction
				// (after down-scaling, unit in pixel) (JPEG only)
	...
	unsigned int    idr_err_skip;       //!< if error in IDR (e.g.slice miss), skip frames until next I frame.
	...
	unsigned int    dither_enable;          // 0: disable dithering, if the stream is 10 bit, the display is 10bit;
	unsigned int    dither_bit;             // for 10bit stream: 0: 8b  1: 10b  2: 16bit

	unsigned int    dbg_log_enable;

	unsigned int    ref_stride;             //if set, will use this to calculate buffer size and configure VCU and PFU, it is of byte unit: address diff between two continuous TILE row;
...
TYPEDEF_STRUCT(vdec_pic_desc_t)
{
	...
	int             ps_left[4];             //!< Horizontal offset (in 1/16 pixels) of the pan-scan window relative to the display window.
	int             ps_top[4];              //!< Vertical offset (in 1/16 pixels) of the pan-scan window relative to the display window.
...
TYPEDEF_STRUCT(vdec_strm_status_t)
{
	...
	unsigned int     strm_pos;      // 29-bit stream position (in bytes)
					// in which the event occurs, if applicable.
	...
	int             hdr_idx;// Index of sequence level headers; Negative if not a
				// sequence level header. <300 if H.264; <8 if other codecs.
...
TYPEDEF_STRUCT(venc_strm_config_t)
{
	...
	void           *hw_context_vaddr;// Virtual address (4KB-aligned) of the stream context buffer for hardware.

	// per-stream parameters necessary for encoding initialization
	unsigned int    src_yuv_format; // YUV format of input video source, e.g. YUV422
	unsigned int    use_1stpass;    // Whether enable first-pass encoding,
					// i.e. 1/4 x 1/4 mostion search
	...
	unsigned int    dis_offset;	//Offset (in bytes) of valid data in a display frame buffer
					//guarantee the offset in chroma is integer.
	unsigned int    dis_stride;	// Stride (in bytes) of the display frame buffer
	unsigned int    dis_buf_size;	// Size (in bytes) of the display frame buffer
					//including Y and UV components.
	...
	unsigned int    max_rate;       // Maximum target bit rate (bps),
					// CBR if (min_rate == max_rate && min_rate == target_rate)
...
TYPEDEF_STRUCT(venc_strm_buf_desc_t)
{
	...
	unsigned int    flags;          // Bit 0 is set if last stream buffer of picture.
			// Bit 1 is set if last stream buffer of slice (bit 1 for JPEG only).
	unsigned int    id;     // Buffer ID assigned by the buffer allocator
	unsigned int    sn;     // Reserved. For video decoder internal use only.
	unsigned int    pos;    // Reserved. For video decoder internal use only.
	unsigned int    ch_id;  // Reserved. For VP8, indicate the channel id (multi-channel)
	unsigned int    part;   // Reserved. For VP8, indicate header(0) or coefficient part(1).
				// For other codec part(2) indicate sequence header(pps/sps.etc)
...
int venc_mch_push_reference_buffer(void *strm_cntxt, venc_vid_buf_desc_t *desc,
				   unsigned int ch_idx);
// buffer management for vp8 multi-channels
int venc_mch_pop_reference_buffer(void *strm_cntxt, venc_vid_buf_desc_t **desc,
				  unsigned int ch_idx);
// buffer management for vp8 multi-channels
int venc_mch_push_video_buffer(void *strm_cntxt, venc_vid_buf_desc_t *desc, unsigned int ch_idx);
// buffer management for vp8 multi-channels
int venc_mch_pop_video_buffer(void *strm_cntxt, venc_vid_buf_desc_t **desc, unsigned int ch_idx);
// buffer management for vp8 multi-channels
